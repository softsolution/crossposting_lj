...
$yescrosspost = $inCore->checkContentAccess($crosslist);
...
if ($do=='config'){
    ...
    if ( !$inCore->inRequest('goadd') ){
		...
		//выключенный кросспостинг
		$smarty->assign('crosspost', $cfg['crosspost']);
		//доступ к кросспостингу
		$smarty->assign('yescrosspost', $yescrosspost);
    ...
    if ( $inCore->inRequest('goadd') ){
		...
		$crosspost 	= $inCore->request('crosspost', 'int', 0);
		$journal 	= $inCore->request('journal', 'str', 'www.livejournal.com');
		$loginlj 	= $inCore->request('loginlj', 'str');
		$passlj 	= $inCore->request('passlj', 'str');

if ($passlj) {//если пришли новые настройки поля пароль шифруем их
$passlj = md5($passlj);
};

		$сommunity 	= $inCore->request('сommunity', 'str');
		$header_loc = $inCore->request('header_loc', 'int', 0);
		$custom_name_on = $inCore->request('custom_name_on', 'int', 0);
		$custom_name = $inCore->request('custom_name', 'str');
		$privacy = $inCore->request('privacy', 'str', 'public');
		$more = $inCore->request('more', 'str', 'lj-cut');
        //Проверяем настройки
        ...
        if(!$error_msg){
            ...
            $blog['seolink_new'] = $model->updateBlog($blog['id'], array('title'=>$title, 'allow_who'=>$allow_who, 'showcats'=>$showcats, 'ownertype'=>$ownertype, 'premod'=>$premod, 'forall'=>$forall, 'crosspost'=>$crosspost, 'journal'=>$journal, 'loginlj'=>$loginlj, 'passlj'=>$passlj, 'сommunity'=>$сommunity, 'header_loc'=>$header_loc, 'custom_name_on'=>$custom_name_on, 'custom_name'=>$custom_name, 'privacy'=>$privacy, 'more'=>$more), $cfg['update_seo_link_blog'], $cfg['crosspost'], $yescrosspost);
...
if ($do=='newpost' || $do=='editpost'){
    ...
    if ( $inCore->inRequest('goadd') ) {
		...
		//$seolink 	= $post['seolink'];//вытаскиваем seolink
		$ljID 		= $post['ljID'];//вытаскиваем ljID
        ...
        if(!$errors){
            ...
            if ($do=='newpost'){
                ...
                $post_id = $model->addPost(array(
                                                    ...
                                                    'tags'=>$tags,
                                                    'comments'=>$comments,
													'crosspost' =>$cfg['crosspost'],
													'yescrosspost'=>$yescrosspost
                                                 ));
            ...
            if ($do=='editpost') {
                ...
                $model->updatePost($post_id, array(
                                                    ...
                                                    'tags'=>$tags,
                                                    'comments'=>$comments,
													//seolink для редактирования поста в ЖЖ
													'seolink'=>$post['postlink'],
													//передаем id блога
													'blog_id'=>$blog['id'],
													//номер поста в ЖЖ
													'ljID'=>$ljID
