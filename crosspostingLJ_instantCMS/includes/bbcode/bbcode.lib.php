    ...
    */
    ...
    function img_2html($elem) {
		...
		if (!strstr($src, 'http://')){				
			if(file_exists($_SERVER['DOCUMENT_ROOT'].$src)){
				...
				if (!$zoom){
				//эта строчка тоже изменена для правильного отображения при кросспостинге
					return '<div class="bb_img" style="'.$div_style.'"><img src="http://'.$_SERVER['HTTP_HOST'].htmlspecialchars($src).'" '.$attr.' /></div>';
				} else {
					$html = '<div class="forum_zoom" style="width:'.$width.'px">'."\n";
					//полный путь при кросспостинге
						$html .= '<div><a href="http://'.$_SERVER['HTTP_HOST'].htmlspecialchars($src).'" target="_blank"><img src="http://'.$_SERVER['HTTP_HOST'].htmlspecialchars($src).'" '.$attr.' width="'.$width.'" height="'.$height.'" border="0"/></a></div>'."\n";
						$html .= '<div class="forum_zoom_text">Изображение уменьшено. Щелкните, чтобы увидеть оригинал.</div>'."\n";
					$html .= '</div>';
					return $html;
				}		
			} else {
			...
			//строчка для правильного отображения картинок в ЖЖ при кросспостинге			
			return '<div class="bb_img" style="'.$div_style.'"><img src="'.htmlspecialchars($src).'" '.$attr.' /></div>';
		}
    }
    ...
    function url_2html($elem) {
        ...
        //Если аттрибут target не указан, считать, что ссылки надо открывать в новом окне. 
        	elseif (!$local)
        	{ $attr .= ' target="_blank"'; }
        return '<a'.$attr.' rel=nofollow>'.$this -> get_html($elem['val']).'</a>';
    }
